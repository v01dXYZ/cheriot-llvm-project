...
static void addCHERICompartmentHashes(const InputFile *file) {
  ...
  for (auto *section : file->getSections()) {
    if (!section || section->name.empty() || !section->contentMaybeDecompress().data())
      continue;

    // Calculate the section hash's digest for specific sections
    if (section->name == ".pcc" || section->name == ".gdc")
      compartment.hashes[section->name] = SHA256::hash(section->contentMaybeDecompress());
  }
}
...
void LinkerDriver::link(opt::InputArgList &args) {
  ...
  // appended to the Files vector.
  {
    ...
    for (size_t i = 0; i < files.size(); ++i) {
      ...
      config->compartments[fileName] = Config::CompartmentSymbols();
